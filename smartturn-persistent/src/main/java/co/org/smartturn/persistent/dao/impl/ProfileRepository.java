...
import co.org.smartturn.persistent.vo.response.ResultVOProfile;
...
public class ProfileRepository extends DataPersistentMysqlHibernate<Profile<Long>, Long> implements ProfileDAO {
	...
	@Override
	public Result<VOProfile> filter(MapEntity filter, Pageable paging) throws PersistentException {
		Session session = null;
		Result<VOProfile> items = null;
	...
	@Override
	public Result<VOProfile> filter(MapEntity filter) throws PersistentException {
		return filter(filter, null);
	}

	@Override
	public boolean update(Profile<?> profile) throws PersistentException {
		return execute((VOProfile) profile, Transaction.UPDATE) != null;
	}
